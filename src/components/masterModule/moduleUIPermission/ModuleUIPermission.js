...
import { searchAssignedPermissions as searchAppAssignedPermissions } from '../../../service/applicationHasUiPermission/ApplicationHasUiPermissionService'
import {
  createModuleHasUiPermission,
  searchModuleAssignedPermissions,
  deleteModuleHasUiPermission,
} from '../../../service/moduleHasUiPermission/ModuleHasUiPermissionService'

const ModuleUIPermission = () => {
  ...
  // Load Assigned Module UI Permissions Mappings
  ...
  // Fetch UI Permissions that can be assigned (mapped to Application but not to Module)
    ...
    try {
      ...
      if (appPermsRes.status === 200 && modPermsRes.status === 200) {
        ...
        const candidates = appMapped
          .map((mapping) => mapping.uiPermission)
        ...
        const unassigned = candidates.filter(
          (cand) =>
            !modMapped.some(
              (m) => m.uiPermissionId === cand.uiPermissionId || m.uiPermission?.uiPermissionId === cand.uiPermissionId
            )
        )
  ...
  const mappingFormSubmit = async (event) => {
    ...
    try {
      const res = await createModuleHasUiPermission(payload)
      if (res.status === 201 || res.status === 200) {
        toast.success(res.message || 'Module UI Permission mapping created successfully!')
  ...
  const handleDeleteMapping = async (mappingId) => {
    try {
      const res = await deleteModuleHasUiPermission(mappingId)
      if (res.status === 200) {
  ...
  const isAllChecked =
    ...
    filteredActivePermissions.map((p) => p.uiPermissionId).every((id) => selectedPermissionIds.includes(id))
    ...
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Manage Module UI Permission</strong>
          </CCardHeader>
          <CCardBody>
                    ...
                    {filteredActivePermissions.length > 0 ? (
                      <div className="row g-2">
                        {filteredActivePermissions.map((perm) => (
                          <CCol xs={12} sm={6} md={4} lg={3} key={perm.uiPermissionId} className="py-1">
                            <div
                      ...
                      <div className="text-center py-5 text-muted small">
                        {mappingSearchParam ? (
                          ...
                          <span>
                            Select Realm, Application, and Module to view unmapped UI permissions.
                          </span>
                        )}
                      </div>
                    )}
            ...
            <CCard className="mt-4 border shadow-sm">
              <CCardHeader>
                <strong>Assigned Module UI Permissions Profiles</strong>
              </CCardHeader>
              <CCardBody>
                {/* Filtration Form */}
                <div
...
export default ModuleUIPermission
